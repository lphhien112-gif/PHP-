...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-top">
        ...
        <button class="drawer-toggle" id="drawerToggle" aria-label="Mở menu" type="button">
            <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
        </button>
    </div>

    <nav class="side-nav" id="sideNav">
        <a href="/dashboard" class="<?= $is('/dashboard') ?>"><?= $icon('dashboard') ?><span>Tổng quan</span></a>
        <a href="/analytics" class="<?= $is('/analytics') ?>"><?= $icon('analytics') ?><span>Thống kê</span></a>
        <a href="/leads" class="<?= $is('/leads') ?>"><?= $icon('leads') ?><span>Lead</span></a>
        <a href="/orders" class="<?= $is('/orders') ?>"><?= $icon('orders') ?><span>Phiếu học phí</span></a>
        <a href="/courses" class="<?= $is('/courses') ?>"><?= $icon('courses') ?><span>Khóa học</span></a>
        <?php if ($user && ($user['role'] ?? '') === 'admin'): ?>
        <a href="/audit" class="<?= $is('/audit') ?>"><?= $icon('audit') ?><span>Nhật ký</span></a>
        <?php endif; ?>
        <a href="/public-leads/create" class="<?= $is('/public-leads') ?>"><?= $icon('public') ?><span>Form công khai</span></a>
    </nav>
    ...
    <div class="sidebar-user">
        ...
        <form method="post" action="/logout" style="margin:0;">
            <?= csrf_field() ?>
            <button type="submit" class="btn-logout" title="Đăng xuất" aria-label="Đăng xuất">
